...
{
    class MaxIndexedPriorityQueue<T> where T : IComparable<T>
    {
        ...
        public MaxIndexedPriorityQueue(int NMax)
        {
            this.NMax = NMax;
            N = 0;
            keys = new T[NMax + 1];
        ...
        void bubbleDown(int k)
        {
            ...
            {
                j = 2 * k;
                if (j < N && less(heap[j], heap[j + 1]))// keys[heap[j]] > keys[heap[j + 1]])
                    j++;

                if (!less(heap[k], heap[j]))//keys[heap[k]] <= keys[heap[j]])
                    break;
